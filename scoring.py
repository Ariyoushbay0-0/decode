...
scoring.py
----------
Scoring Engine (بند ۲).
به هر خروجی Decode یک امتیاز عددی می‌دهد که ترکیبی است از:
- درصد کاراکترهای printable
- خوانا بودن متن (نسبت حروف/فاصله به کل)
- وجود الگوهای رایج Flag
- وجود کاراکترهای معمول Flag (_ - و الفبا/اعداد)
- جریمه برای کاراکترهای خراب / باینری گارباژ
- جریمه بر اساس Entropy بالا (نشانه‌ی متن تصادفی/رمزنگاری‌شده)
این امتیاز هم در heuristic جستجو (h(n)) و هم در رتبه‌بندی Flagهای نهایی استفاده می‌شود.
"""
...
FLAG_PATTERNS = [
    ...
    r"flag\{[^{}]{1,300}\}",
    r"CTF\{[^{}]{1,300}\}",
    r"[A-Za-z0-9_]{2,20}\{[^{}]{1,300}\}",  # الگوی عمومی PREFIX{...}
]
